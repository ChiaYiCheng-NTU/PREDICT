...
def main():
    ...
    while True:
        if i < 10:
            pre_destination_dir = f"{os.path.dirname(os.path.realpath(__file__))}/../../../Results/{formatted_date}_FindKmers0{i}"
            if not os.path.exists(pre_destination_dir):
                destination_dir = pre_destination_dir
                break
        elif i >= 10:
            pre_destination_dir = f"{os.path.dirname(os.path.realpath(__file__))}/../../../Results/{formatted_date}_FindKmers{i}"
            if not os.path.exists(pre_destination_dir):
                destination_dir = pre_destination_dir
                break
        i += 1
