...
def main(gff, genome, tp, tn):
    ...
    i = 1
    while True:
        pre_destination_dir = f"{os.path.dirname(os.path.realpath(__file__))}/../../../Results/{formatted_date}_KmerImpact{i:02d}"
        if not os.path.exists(pre_destination_dir):
            destination_dir = pre_destination_dir
            break
        i += 1

    source_dir = f'{os.path.dirname(os.path.realpath(__file__))}/../../../InputData/KmerImpact'
